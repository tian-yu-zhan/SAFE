...
library(Matrix)
library(doParallel)
setwd("~/1_doc/Research/AbbVie/Safety/1_code/v2/2_sim_2_power/")
...
final.out.mat = matrix(NA, nrow = 28, ncol = 2*n.r+6)
...
for (scen.ind in c(1:28)){
  ...
  # for (itt in 1:n.itt){
    ...
    p.1st.bon.vec = sapply(1:n.r, function(x.r){
      sort.temp = as.numeric(data.pvalue.temp[x.r, ])
      index.2nd = (order(sort.temp))[1]
      p.adj.temp = p.adjust(sort.temp, method = "holm")
      return(p.adj.temp[index.2nd])
    })
    
    ## FDR control across n.r regions
    p.FDR.alt.vec = p.adjust(p.1st.bon.vec, method = "BH")
    
    if (sum(p.FDR.alt.vec<=alpha)>0){
      FDR.out.alt.temp = 
        sum(p.FDR.alt.vec[(n.r.false+1):n.r]<=alpha)/sum(p.FDR.alt.vec<=alpha)
    } else {
      FDR.out.alt.temp = 0
  ...
  output.mat.temp = matrix(unlist(output.vec.temp),
                           nrow = n.itt, ncol = 2*(n.r+1), byrow = TRUE)
...
colnames(final.out.mat) = c("mean.ind", "n.r.false", "cor.stats.num.r", 
                            "cor.stats.num.v",
                            "v1", "v2", "v3", "v4", "v5", "FDR",
                            "v1.alt", "v2.alt", "v3.alt", "v4.alt", "v5.alt", 
                            "FDR.alt")

print(final.out.mat)

write.csv(final.out.mat, paste0("safe_power_nr_", n.r, ".csv"))
}
...
data.output.1 = data.frame(read.csv("safe_power_nr_5.csv")[,-1])
...
data.output$h1.m2 = paste0(sprintf('%.1f', data.output.1$v1.alt*100), "%")
data.output$FDR.m2 = paste0(sprintf('%.1f', data.output.1$FDR.alt*100), "%")
