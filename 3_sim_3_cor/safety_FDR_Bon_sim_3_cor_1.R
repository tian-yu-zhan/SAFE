...
library(Matrix)
library(doParallel)
setwd("~/1_doc/Research/AbbVie/Safety/1_code/v2/3_sim_3_cor/")
...
for (scen.ind in 1:28){
  ...
  if (cor.r.ind == 3) cor.stats.num.r = 0.3  
  ...
  stats.sigma.mat = matrix(0, nrow = n.v, ncol = n.v)
  stats.sigma.mat[1:7, 1:7] = cor.stats.num.v
  stats.sigma.mat[8:15, 8:15] = cor.stats.num.v
  diag(stats.sigma.mat) = 1
...
write.csv(final.out.mat, paste0("safe_cor_1_nr_", n.r, ".csv"))
}
...
data.output = data.frame(read.csv("safe_cor_1_nr_5.csv")[,-1])
...
data.output$V3[data.output$V3=="0.300"] = "0.3"
...
data.output$V5[15:28] = "-"
data.output$V6[22:28] = "-"
